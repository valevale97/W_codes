...
% functions in both velocity space and energy-pitch space similarly to the 
% original script. However, this script plots the log10 value og the
% probability function to use more of the colorbar
...
v_perpendicular = (dv:dv:2.5+dv)*1e7;
...
wf_neutron_array_log10 = zeros(sizeA,sizeB,length(E_n));

wf_neutron_array_E_p = zeros(length(pitch),length(E_d),length(E_n));
wf_neutron_array_E_p_log10 = zeros(length(pitch),length(E_d),length(E_n));
...
for E = E_n;
    ...
    wf_neutron_array_E_p_log10(:,:,E_counter) = log10(weight_function_E_p);
    ...
    wf_neutron_array_log10(:,:,E_counter) = log10(weight_function);
    end
    E_counter = E_counter + 1;
end

wf_neutron_array_log10(isinf(wf_neutron_array_log10)) = -3;
wf_neutron_array_E_p_log10(isinf(wf_neutron_array_E_p_log10)) = -3;
...
if 1
    ...
    for l = j
        ...
        figure
        [~,h] = contourf(v_parallel/1e6,v_perpendicular/1e6,wf_neutron_array_log10(:,:,l),50);
        ...
        %saveas(gcf,strcat('results\Article_2\wf_plots\wf_v_space_phi_', num2str(phi_degrees), '_En_', E_n_plots(E_n_plot_counter,:), '_MeV_log10.png'))
        %saveas(gcf,strcat('results\Article_2\wf_plots\wf_v_space_phi_', num2str(phi_degrees), '_En_', E_n_plots(E_n_plot_counter,:), '_MeV_log10.eps'),'epsc')
        
        %saveas(gcf,strcat('results\Article_2\wf_plots\wf_v_space_phi_', num2str(phi_degrees), '_En_', E_n_plots(E_n_plot_counter,:), '_MeV_log10_referee_comments.png'))
        %saveas(gcf,strcat('results\Article_2\wf_plots\wf_v_space_phi_', num2str(phi_degrees), '_En_', E_n_plots(E_n_plot_counter,:), '_MeV_log10_referee_comments.eps'),'epsc')
        
        figure
        [~,h] = contourf(E_d/1.6e-19/1e6,pitch,wf_neutron_array_E_p_log10(:,:,l),50);
        set(h,'edgecolor','none')
        axis square
        axis tight
        ylim([-1 1])
        ...
        %saveas(gcf,strcat('results\Article_2\wf_plots\wf_E_p_phi_', num2str(phi_degrees), '_En_', E_n_plots(E_n_plot_counter,:), '_MeV_log10_new.png'))
        %saveas(gcf,strcat('results\Article_2\wf_plots\wf_E_p_phi_', num2str(phi_degrees), '_En_', E_n_plots(E_n_plot_counter,:), '_MeV_log10_new.eps'),'epsc')
        
        %saveas(gcf,strcat('results\Article_2\wf_plots\wf_E_p_phi_', num2str(phi_degrees), '_En_', E_n_plots(E_n_plot_counter,:), '_MeV_log10_referee_comments.png'))
        %saveas(gcf,strcat('results\Article_2\wf_plots\wf_E_p_phi_', num2str(phi_degrees), '_En_', E_n_plots(E_n_plot_counter,:), '_MeV_log10_referee_comments.eps'),'epsc')
